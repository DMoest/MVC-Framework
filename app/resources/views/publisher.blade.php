<x-layout>
    <x-section>
        ...
        <div class="w-3/4 m-auto my-10 p-4">
            <div class="font-body">
                <h2 class="text-xl text-center font-weight-bold font-header pb-6">{{ $publisher->name }}</h2>

                <p class="py-1 text-center">This is the part of {{ $publisher->name }}'s work I curently own.</p>
            </div>
            ...
            <x-table>
                ...
                @foreach ($books as $key => $book)
                    <x-table-row class="hover:cursor-pointer">
                        ...
                        <x-td-link route="./../book/{{ $book->id }}"><p class="px-2 m-0 text-center"> {{ $book->author->name }} </p></x-td-link>
